...
% --- 2. 读取指定目录下的 CSV 文件 ---
data_dir = 'D:\自制软件\1.thesis\My_nju_thesis-master\My_nju_thesis-master\验证推演3.15\腔实际性能参数验算\光腰为0.88mm测得数据\原始数据\';
output_dir = 'D:\自制软件\1.thesis\My_nju_thesis-master\My_nju_thesis-master\验证推演3.15\腔实际性能参数验算\程序绘图\输出图\';
...
for i = 1:length(files)
    fname = files(i).name;
    % 正则表达式提取文件名中的 l 值 (匹配 DATA70_l0_symmetrized.csv 格式)
    tokens = regexp(fname, 'l(\d+)', 'tokens');
    if isempty(tokens)
        continue;
    end
...
% --- 6. 导出高质量 PDF ---
output_pdf = fullfile(output_dir, 'transmission_3d_plot.pdf');
fprintf('正在导出高质量 PDF 到: %s ...\n', output_pdf);
