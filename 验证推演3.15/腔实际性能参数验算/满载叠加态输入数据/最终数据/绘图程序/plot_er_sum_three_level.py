...
def plot_three_level(
    ...
    lower = ax.axhline(
        ...
        linewidth=1.0,
        zorder=4.5,
        label=r"Lorentz benchmark ($10.47\ \mathrm{dB}$)",
    )
    ...
    y_max = max(float(np.max(exp_mean + exp_sd)), float(np.mean(airy_theory)), ER_INF_LORENTZ)
    ax.set_ylim(0, math.ceil((y_max + 1.0) / 2) * 2)
    ...
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.85), pad=0.45)
